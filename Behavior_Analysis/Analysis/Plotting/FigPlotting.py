...
def plotFig1B(file_base, pic_base, monkey):
    print("----------- Fig 1B -----------")
    # for n in ["trial", "game", "day"]:
    for n in ["day"]:
        with open("{}/{}_1.B.{}.pkl".format(file_base, monkey, n), "rb") as file:
            result_df_list = pickle.load(file)
        ...
        for i, c in enumerate(["straight", "L-shape", "fork", "cross"]):
            gpd_list = [result_df[result_df.category == c] for result_df in result_df_list]
            trial_mean = np.zeros((len(gpd_list), np.max([each.shape[0] for each in gpd_list])))
            trial_mean[trial_mean == 0.0] = np.nan
            for t_idx, gpd in enumerate(gpd_list):
                trial_mean[t_idx, :gpd.shape[0]] = gpd["mean"].values
            avg_val = np.nanmean(trial_mean, axis=0)
            # std_val = scipy.stats.sem(trial_mean, axis=0, nan_policy="omit")
            std_val = np.nanstd(trial_mean, axis=0)
            # std_val = np.nanstd(trial_mean, axis=0)
            ax.plot(
                range(len(avg_val)),
                avg_val,
        ...
        plt.tight_layout()
        plt.savefig("{}/".format(pic_base) + monkey + "/1.B.{}.pdf".format(n))
        plt.show()
    # -----
...
def plotFig1C(file_base, pic_base, monkey):
    print("----------- Fig 1C -----------")
    dd = {
    ...
    # for n in ["trial", "game", "day"]:
    for n in ["day"]:
        with open("{}/{}_1.C.{}.pkl".format(file_base, monkey, n), "rb") as file:
        ...
        for c in ['ghost(red) normal', 'ghost(red) scared', 'ghost(yellow) normal', 'ghost(yellow) scared']:
            gpd_list = [each[each.ifscared1 == c] for each in rs_list]
            gpd_mean = np.zeros((len(gpd_list), 26))
            gpd_mean[gpd_mean == 0.0] = np.nan
            for t_idx, gpd in enumerate(gpd_list):
                try:
                    gpd_mean[t_idx, gpd.level_0.apply(lambda x: int(x)).values] = gpd["mean"].values
                except:
                    continue
            avg_val = np.nanmean(gpd_mean, axis=0)
            # std_val = scipy.stats.sem(gpd_mean, axis=0, nan_policy="omit")
            std_val = np.nanstd(gpd_mean, axis=0)
        ...
        plt.tight_layout()
        plt.savefig("{}/".format(pic_base) + monkey + "/1.C.{}.pdf".format(n))
        plt.show()
        plt.close()
    # -----
...
def plotFig2A(file_base, pic_base):
    df_monkey = pd.read_pickle("{}/example_trial.pkl".format(file_base))
    ...
    all_list = ind_list + ["weight"]
    contributions = df_monkey.contribution
    is_correct = df_monkey.is_correct
    fitted_label = df_monkey.labels
    model_dir = df_monkey[all_list].apply(lambda x: _getDir(np.array(x.weight) @ _2Array(x[ind_list])), axis=1)
    actual_dir = pd.Series([_moveDir(df_monkey.pacmanPos[i], df_monkey.pacmanPos[i + 1]) for i in
                  range(df_monkey.shape[0] - 1)] + [np.nan])
    actual_dir = list(actual_dir.fillna(method="ffill").fillna(method="bfill").values)
    # actual_dir = data.move_dir
    if part is True:
        if trial_name == '23-1-Omega-21-Aug-2019-1.csv':
            ind = np.arange(83, 130)
            offset = 84
        elif trial_name == '22-1-Omega-26-Jul-2019-1.csv':
            ind = np.arange(2, 40)
            offset = 3
        elif trial_name == '25-1-Omega-17-Jul-2019-1.csv':
            # ind = np.arange(2, 40)
            # offset = 3
            ind = np.arange(5, 40)
            offset = 6
        elif trial_name == '31-1-Patamon-13-Jul-2019-1.csv':
            ind = np.arange(141, 187)
            offset = 142
        elif trial_name == '1-1-Patamon-25-Jun-2019-1.csv':
            ind = np.arange(0, 67)
            offset = 1
        elif trial_name == '8-4-Patamon-05-Jul-2019-1.csv':
            ind = np.arange(47, len(is_correct))
            offset = 48
        elif trial_name == '21-3-Patamon-02-Sep-2019-1.csv':
            ind = np.arange(16, len(is_correct))
            offset = 17
    else:
        ind = np.arange(len(is_correct))
        offset = 1
    # split into phases
    sep = np.load(
        "../../Data/refit_plot_data/all_trials-merge_weight-dynamic-bkpt.npy",
        allow_pickle=True)
    for i in sep:
        if i[0] == trial_name:
            trial_sep = i[2]
            break
    # -----
    df_monkey = df_monkey.iloc[ind].reset_index(drop=True)
    contributions = contributions[ind].reset_index(drop=True)
    is_correct = is_correct[ind].reset_index(drop=True)
    fitted_label = fitted_label[ind].reset_index(drop=True)
    model_dir = np.array(model_dir)[ind]
    actual_dir = np.array(actual_dir)[ind]
    # ---------------------
    ...
    for j in np.arange(6):
        plt.plot([i[j] for i in contributions.values], color=status_color_mapping[agents[j]], ms=3, lw=5,
                 label=label_name[agents[j]])
    # plt.title(trial_name, fontsize=13)
    plt.ylabel("Strategy Weight", fontsize=30)
    plt.xlim(0, df_monkey.shape[0] - 1)
    x_ticks_index = np.arange(0, len(df_monkey), 10)
    x_ticks = [offset + int(each) for each in x_ticks_index]
    plt.xticks(list(x_ticks_index) + [len(df_monkey)], x_ticks + [len(df_monkey) + offset], fontsize=20)
    plt.yticks(fontsize=20)
    plt.ylim(-0.01, 1.02)
    plt.legend(loc="upper center", fontsize=23, ncol=len(agent_name), frameon=False, bbox_to_anchor=(0.5, 1.2))

    estimated_label = fitted_label
    ax2 = fig.add_subplot(spec[2, :])
    for i in range(len(estimated_label)):
        plt.fill_between(x=[i, i + 1], y1=0.0, y2=1.0, color=status_color_mapping[estimated_label[i]])
    ...
    for i in range(len(actual_dir)):
        tmp_dir = actual_dir[i]
        another_dir = model_dir[i]
        if tmp_dir == another_dir:
            col = "k"
        else:
            col = "r"
        if tmp_dir == "right":
        ...
        # if dy == 0:
        #     plt.arrow(x, y, dx, dy, width = 0.02 , head_length = 0.06, head_width = 0.12, shape = "full", length_includes_head = True, ec = col, fc = col)
        # else:
        #     plt.arrow(x, y, dx, dy, width=0.035, head_length = 0.06, head_width = 0.20, shape="full", length_includes_head=True, ec=col, fc=col)

        an = ax3.annotate('', xy=(x + dx, y + dy), xytext=(x, y), arrowprops=dict(width=0.01), fontsize=8)
        an.arrow_patch.set_color(col)
        an.arrow_patch.set_arrowstyle("->", head_width=0.05, head_length=0.1)

    # ax4 = fig.add_subplot(spec[4, :])
    for i in range(len(model_dir)):
        tmp_dir = model_dir[i]
        another_dir = actual_dir[i]
        if tmp_dir == another_dir:
            col = "k"
        else:
            col = "r"
    ...
    plt.xlim(0, len(actual_dir))
    plt.ylim(0.1, 1.5)
    plt.axis('off')

    ax3.text(-0.05, 0.65, 'Monkey:', rotation="horizontal",
             verticalalignment='center', horizontalalignment='center',
             transform=ax3.transAxes,
             color='k', fontsize=30)
    ax3.text(-0.045, 0.25, 'Model: ', rotation="horizontal",
             verticalalignment='center', horizontalalignment='center',
             transform=ax3.transAxes,
             color='k', fontsize=30)
    plt.savefig("{}/".format(pic_base) + "2A.example_trial.pdf")
    plt.show()
    print("Correct rate : ", np.nanmean(actual_dir == model_dir))


def plotFig2B(file_base, pic_base, monkey):
    print("----------- Fig 2B -----------")
    # read data
    width = 0.15
    random_data = np.load("{}/100trial-all_random_is_correct.npy".format(file_base), allow_pickle=True).item()
    ...
    avg_hybrid_cr, std_hybrid_cr, avg_moving_cr, std_moving_cr, avg_preceptron_cr, std_perceptron_cr, type_name = \
        np.load("{}/{}_2B.npy".format(file_base, monkey), allow_pickle=True)
    avg_q_cr, std_q_cr = np.load("{}/{}_2B.q_learning.npy".format(file_base, monkey), allow_pickle=True)
    print("-" * 70)
    print("Moving model : ")
    print("Avg: ", avg_moving_cr)
    print("Std: ", std_moving_cr)
    print("-" * 70)
    print("Hybrid model : ")
    print("Avg: ", avg_hybrid_cr)
    print("Std: ", std_hybrid_cr)
    print("-" * 70)
    ...
    print("Avg: ", avg_preceptron_cr)
    print("Std: ", std_perceptron_cr)
    # --------------------
    ...
    ax.spines['right'].set_visible(False)
    ax.spines['top'].set_visible(False)
    plt.bar(x=np.arange(0, 4) + width, height=[avg_moving_cr[each] for each in type_name], width=width,
    ...
    plt.bar(x=np.arange(0, 4), height=[avg_hybrid_cr[each] for each in type_name], width=width,
            ...
            error_kw={"capthick": 3, "elinewidth": 3})
    plt.bar(x=np.arange(0, 4) - width, height=[avg_q_cr[each] for each in type_name], width=width,
            label="LARL",
            color=light_grey,
            linewidth=2,
            edgecolor=black,
            yerr=[std_q_cr[each] for each in type_name], capsize=7,
            error_kw={"capthick": 3, "elinewidth": 3})
    ...
    plt.xticks(np.arange(0, 4) - width/2.0, [type_name[each] for each in type_name], fontsize=20)
    plt.ylim(0.4, 1.15)
    ...
    plt.tight_layout()
    plt.savefig(pic_base + "/2B.{}.with_Q_learning.pdf".format(monkey))
    plt.show()


def plotFig2CDE(file_base, pic_base, monkey):
    print("---------- Fig 2CDE ----------")
    # Extract data
    ...
    print("1st - 2nd Weight: {} larger than 0.1, {} larger than 0.9".format(
        len(first_second_diff[first_second_diff > 0.1]) / len(first_second_diff),
        len(first_second_diff[first_second_diff > 0.9]) / len(first_second_diff)))
    # Histogram; Probability
    bin = np.arange(0, 1.1, 0.1)
    first_weight_bin, bin_edges = np.histogram(first_weight, bin)
    first_weight_bin = np.random.uniform(np.repeat(bin_edges[:-1], first_weight_bin),
                                         np.repeat(bin_edges[1:], first_weight_bin))
    second_weight_bin, bin_edges = np.histogram(second_weight, bin)
    second_weight_bin = np.random.uniform(np.repeat(bin_edges[:-1], second_weight_bin),
                                          np.repeat(bin_edges[1:], second_weight_bin))
    third_weight_bin, bin_edges = np.histogram(third_weight, bin)
    third_weight_bin = np.random.uniform(np.repeat(bin_edges[:-1], third_weight_bin),
                                         np.repeat(bin_edges[1:], third_weight_bin))
    diff_weight_bin, bin_edges = np.histogram(first_second_diff, bin)
    diff_weight_bin = np.random.uniform(np.repeat(bin_edges[:-1], diff_weight_bin),
                                        np.repeat(bin_edges[1:], diff_weight_bin))
    # Configurations
...
def plotFig2F(file_base, pic_base, monkey):
    print("----------- Fig 2F -----------")
    ...
    for i in x_index:
        for j, a in enumerate(agent_name):
            if a != "evade":
                d = 0.0
                if agent_name[j] in data[type[i]]:
                    d = data[type[i]][agent_name[j]]
            else:
                d = 0.0
                if "evade_blinky" in data[type[i]]:
                    d += data[type[i]]["evade_blinky"]
                if "evade_clyde" in data[type[i]]:
                    d += data[type[i]]["evade_clyde"]
            if i == 0:
                plt.bar(i, d,
...
def plotFig2FRadar(file_base, pic_base, monkey):
    print("----------- Fig 2F -----------")
    with open("{}/{}_2F.pkl".format(file_base, monkey), "rb") as file:
        data = pickle.load(file)
    # type = ["all", "early", "end", "close-scared", "close-normal"]
    type = ["all", "early", "end", "close-scared"]
    type_name = {
        "middle": "Middle Stage",
        "close-normal": "Normal Ghosts",
        "all": "Overall", "early": "Early Game",
        "end": "Late Game", "close-scared": " Scared Ghosts"
    }
    agent_name = ["global", "local", "evade", "approach", "energizer", "vague"] + ["global"]
    # agent_name = ["global", "local", "evade", "approach", "energizer"] + ["global"]
    context_color_mapping = {
        # "middle": "#505099",
        "close-normal": status_color_mapping["evade"],
        "all": light_grey,
        "end": status_color_mapping["global"],
        "early": status_color_mapping["local"],
        "close-scared": status_color_mapping["approach"],
    }
    # ----------------
    plt.figure(figsize=(8, 5))
    ax = plt.subplot(polar=True)
    agent_circle_loc = np.linspace(start=0, stop=2 * np.pi, num=len(agent_name))
    for t in type:
        context_data = data[t]
        context_agent_ratio = [
            context_data["global"],
            context_data["local"],
            context_data["evade_blinky"] + context_data["evade_clyde"],
            context_data["approach"],
            context_data["energizer"],
            context_data["vague"],
        ]
        context_agent_ratio = context_agent_ratio + [context_agent_ratio[0]]
        plt.plot(agent_circle_loc, context_agent_ratio, "o-", lw=3, label=type_name[t], color=context_color_mapping[t])
        plt.fill(agent_circle_loc, context_agent_ratio, color=context_color_mapping[t], alpha=0.5)
    plt.thetagrids(np.degrees(agent_circle_loc), labels=agent_name)
    ax.tick_params(rotation='auto', labelsize=15)
    plt.ylim(0.0, 0.6)
    ax.set_rlabel_position(np.linspace(start=0, stop=361, num=len(agent_name))[4] + 5)
    plt.yticks([0.2, 0.4, 0.6], [0.2, 0.4, 0.6], color="grey", size=13)
    plt.legend(fontsize=15, frameon=False, bbox_to_anchor=(1.45, 0.3))
    plt.tight_layout()
    plt.savefig("{}/".format(pic_base) + monkey + "/2.F.radar.pdf")
    plt.show()
...
def plotFig3AB(file_base, pic_base, monkey):
    ...
    for t in range(game_beans_data.shape[0]):
        for r in range(repeat_num):
            tmp = [0 for _ in range(7)]
            # tmp = [0 for _ in range(len(game_beans_data))]
...
def plotFig3CD(file_base, pic_base, monkey):
    print("----------- Fig 3C -----------")
    ...
    ax.spines['right'].set_visible(False)
    ax.spines['top'].set_visible(False)
    sns.heatmap(cnt_bin, square=False, cmap="binary", cbar=False, linewidth=0.5, annot=cnt_size,
                annot_kws={"fontsize": 15}, fmt=".1%")
...
def plotFig3E(file_base, pic_base, monkey):
    print("----------- Fig 3E -----------")
...
def plotFig3F(file_base, pic_base, monkey, std_coef = 3):
    print("----------- Fig 3F -----------")
    with open("{}/{}_3F_{}std.pkl".format(file_base, monkey, std_coef), "rb") as file:
        data = pickle.load(file)
    ...
    for index, t_type in enumerate(trans_):
        ...
        plt.tight_layout()
        plt.savefig("{}/".format(pic_base) + monkey + "/3F_{}std.{}_{}_{}.pdf".format(std_coef, index + 1, t_type[0], t_type[1]))
        plt.show()
        ...
        if test[i] < 0.01:
            ...
            if i == 0:
                ...
                ax.add_line(lines.Line2D([x_index[i] + interval, x_index[i] - interval],
                                         [min_y - gap, min_y - gap], lw=8., color='k'))
    plt.tight_layout()
    plt.savefig("{}/".format(pic_base) + monkey + "/3F_{}std.all.pdf".format(std_coef))
    plt.show()
...
def plotFig5A(file_base, pic_base, monkey):
    print("----------- Fig 5A -----------")
    plt.figure(figsize=(6, 5))
    ax1 = plt.subplot(1, 1, 1)
    ax1.spines['right'].set_visible(False)
    ax1.spines['top'].set_visible(False)
    plt.title("Planned Attack", fontsize=20, y=1.25)
    with open("{}/{}_5A_planned_weight.pkl".format(file_base, monkey), "rb") as file:
        data = pickle.load(file)
    ...
    plt.ylabel("Strategy Weight", fontsize=20)
    plt.tight_layout()
    plt.savefig("{}/".format(pic_base) + monkey + "/5A.planned.pdf")
    plt.show()
    ...
    plt.title("Accidental Consumption", fontsize=20, y=1.25)
    with open("{}/{}_5A_accidental_weight.pkl".format(file_base, monkey), "rb") as file:
        data = pickle.load(file)
    mean_data = np.nanmean(data, axis=0)
    sem_data = scipy.stats.sem(data, axis=0, nan_policy="omit")
    tmp_agent_name = copy.deepcopy(agent_name)
    ...
    plt.ylabel("Strategy Weight", fontsize=20)
    plt.tight_layout()
    plt.savefig("{}/".format(pic_base) + monkey + "/5A.accidental.pdf")
    plt.show()


def plotFig56CD(file_base, pic_base, monkey):
    # ===========================================================================
    print("----------- Fig 5C -----------")
    with open("{}/{}_5C.ghost_all.pkl".format(file_base, monkey), "rb") as file:
        detail_data = pickle.load(file)
    ...
    print("| Planned vs. Accidental Saccade Ratio T Test | Ghost = {} / Energizer = {} / Pacman = {}".format(test[0][1],
                                                                                                             test[1][1],
                                                                                                             test[2][
                                                                                                                 1]))
    ...
    identity_sacc_std = {
        "ghost_all": [scipy.stats.sem(planned_data[0], nan_policy="omit"),
                      scipy.stats.sem(accidental_data[0], nan_policy="omit")],
        "energizer_sacc": [scipy.stats.sem(planned_data[1], nan_policy="omit"),
                           scipy.stats.sem(accidental_data[1], nan_policy="omit")],
        "pacman_sacc": [scipy.stats.sem(planned_data[2], nan_policy="omit"),
                        scipy.stats.sem(accidental_data[2], nan_policy="omit")],
    }
    ...
    plt.tight_layout()
    plt.savefig('{}/{}/5C_ratio.pdf'.format(pic_base, monkey))
    plt.show()
    plt.close()
    # ===========================================================================
    print("----------- Fig 5D -----------")
    with open("{}/{}_5D.detail.pkl".format(file_base, monkey), "rb") as file:
        detail_data = pickle.load(file)
    ...
    test = [
        scipy.stats.ttest_ind(planned_data[i][~np.isnan(planned_data[i])],
                              accidental_data[i][~np.isnan(accidental_data[i])])
        for i in range(len(planned_data))
    ]
    print("| Planned vs. Accidental Pupil Size T Test (time step) | ", [i[1] for i in test])
    with open("{}/{}_5D.pkl".format(file_base, monkey), "rb") as file:
        data = pickle.load(file)
    fig = plt.figure(figsize=(6, 5))
    ax = fig.add_subplot()
    ax.spines['right'].set_visible(False)
    ...
    plt.fill_between([-1, 9], [min_mean_data - 0.1, min_mean_data - 0.1], [max_mean_data + 0.1, max_mean_data + 0.1],
                     color=dark_grey, linewidth=0.0, alpha=0.1)
    plt.ylabel("Normalized Pupil Size", fontsize=20)
    ...
    for i in range(len(test)):
        if test[i][1] < 0.01:
            ...
            if i == 0:
                ax.add_line(
                    lines.Line2D([x_index[i] + 1, x_index[i]], [min_mean_data - 0.1, min_mean_data - 0.1], lw=8.,
                                 color='k'))
            elif i == len(test) - 1:
                ax.add_line(
                    lines.Line2D([x_index[i], x_index[i] - 1], [min_mean_data - 0.1, min_mean_data - 0.1], lw=8.,
                                 color='k'))
            else:
                ax.add_line(
                    lines.Line2D([x_index[i] + 1, x_index[i] - 1], [min_mean_data - 0.1, min_mean_data - 0.1], lw=8.,
                                 color='k'))
    plt.tight_layout()
    plt.savefig('{}/{}/5D_pupil.pdf'.format(pic_base, monkey))
    plt.show()
    plt.close()
    # ===========================================================================
    print("----------- Fig 6C -----------")
    with open("{}/{}_6C.ghost_all.pkl".format(file_base, monkey), "rb") as file:
        detail_data = pickle.load(file)
    ...
    print("| Suicide vs. Failure Saccade Ratio T Test | Ghost = {} / Pellets = {} / Pacman = {}".format(test[0][1],
                                                                                                        test[1][1],
                                                                                                        test[2][1]))
    colors = [dark_grey, light_grey]
    identity_map = {"ghost1Pos_sacc": "Ghost Blinky", "ghost2Pos_sacc": "Ghost Clyde", "pacman_sacc": "Pacman",
    ...
    plt.yticks(fontsize=20)
    if monkey == "Omega":
        plt.ylim(0.0, np.max(list(identity_sacc_mean.values())) + 0.08)
    else:
        plt.ylim(0.0, np.max(list(identity_sacc_mean.values())) + 0.05)
    if monkey == "Patamon" or monkey == "Omega":
        # plt.legend(["Suicide", "Failure"], fontsize=18, loc="upper left")
        plt.legend(["Suicide", "Failed evasion"], fontsize=18, loc="upper left")
    else:
        # plt.legend(["Suicide", "Failure"], fontsize=18)
        plt.legend(["Suicide", "Failed evasion"], fontsize=18)
    for i in range(len(test)):
        tmp_ks = test[i]
        if tmp_ks[1] < 0.001:
            t = "***"
        ...
        plt.text(ks_x_index[i], ks_y[i], t, fontsize=20, ha="center", va="center")
    plt.tight_layout()
    plt.savefig('{}/{}/6C_ratio.pdf'.format(pic_base, monkey))
    plt.show()
    plt.close()
    # ===========================================================================
    print("----------- Fig 6D -----------")
    with open("{}/{}_6D.detail.pkl".format(file_base, monkey), "rb") as file:
        detail_data = pickle.load(file)
    suicide_data = detail_data["suicide"]
    ...
    test = [
        scipy.stats.ttest_ind(suicide_data[i][~np.isnan(suicide_data[i])], normal_data[i][~np.isnan(normal_data[i])])
        for i in range(len(suicide_data))]
    print("| Suicide vs. Falure Pupil Size T Test (time step) | ", [i[1] for i in test])
    with open("{}/{}_6D.pkl".format(file_base, monkey), "rb") as file:
        data = pickle.load(file)
    fig = plt.figure(figsize=(6, 5))
    ...
    type_map = {"suicide": "Suicide", "normal": "Failed evasion"}
    max_mean_data = -99999
    min_mean_data = 99999
    for index, each in enumerate(type):
        # 合并数据
    ...
    plt.tight_layout()
    plt.savefig('{}/{}/6D_pupil.pdf'.format(pic_base, monkey))
    plt.show()
    plt.close()


def plotFig5B6AB(file_base, pic_base, monkey):
    # ===========================================================================
    print("----------- Fig 5B -----------")
    with open("{}/{}_5B.all.pkl".format(file_base, monkey), "rb") as file:
        data = pickle.load(file)
    ...
    # plt.yticks([-0.15, -0.1, -0.05, 0.0, 0.05, 0.1, 0.15], [0.15, 0.1, 0.05, 0.0, 0.05, 0.1, 0.15], fontsize=15) #TODO: monkey
    ...
    plt.tight_layout()
    plt.savefig("{}/".format(pic_base) + monkey + "/5B.PA_vs_AA.avg.pdf")
    plt.show()
    # ===========================================================================
    print("----------- Fig 6A -----------")
    with open("{}/{}_6A.detail.pkl".format(file_base, monkey), "rb") as file:
        detail_data = pickle.load(file)
    ...
    plt.bar(bins[:-1], -evade_data_bin / np.sum(evade_data_bin), width=step - 0.15, color=light_grey,
            alpha=1.0, align="edge", label="Failed evasion")
    ...
    ax.text(ax.get_xlim()[1], 0.0, t, fontsize=22, ha="center",
            va="center", rotation=90)
    plt.tight_layout()
    plt.savefig("{}/".format(pic_base) + monkey + "/6A.suicide_vs_evade.reset.pdf")
    plt.show()
    # ===========================================================================
    print("----------- Fig 6B -----------")
    with open("{}/{}_6B.detail.PG.pkl".format(file_base, monkey), "rb") as file:
    ...
    plt.ylabel("Probability", fontsize=20)
    # plt.yticks([-0.2, -0.1, 0.0, 0.1, 0.2], [0.2, 0.1, 0.0, 0.1, 0.2], fontsize=15) #TODO: monkey
    plt.xticks(fontsize=15)
    plt.xlim(0, 30)
    ...
    ax.text(ax.get_xlim()[1], 0.0, t, fontsize=22, ha="center",
            va="center", rotation=90)
    plt.tight_layout()
    plt.savefig("{}/".format(pic_base) + monkey + "/6B.suicide_vs_evade.PG.pdf")
    plt.show()
...
def plotBasicStatistics(file_base, pic_base):
    print("---------- Supp Fig ----------")
    # Read records
    ...
    omega_round_eaten_ghost_all = [np.sum(omega_record["round_eaten_ghosts"][each]) for each in
                                   omega_record["round_eaten_ghosts"]]
    patamon_round_eaten_ghost_all = [np.sum(patamon_record["round_eaten_ghosts"][each]) for each in
                                     patamon_record["round_eaten_ghosts"]]
    ...
    plt.text(x=np.mean(omega_round_eaten_ghost_all), y=ax.get_ylim()[1] + 15,
             s="{:.1f} ($\pm$ {:.1f})".format(np.mean(omega_round_eaten_ghost_all),
                                              np.std(omega_round_eaten_ghost_all)),
             fontsize=18, ha="center", va="center")
    ...
    patamon_num_game_per_day = {each: len(patamon_record["game_per_day"][each]) for each in
                                patamon_record["game_per_day"]}
    ...
    omega_round_std = {each: np.std(omega_round_per_game_per_day[each]) / len(omega_round_per_game_per_day[each]) for
                       each in omega_round_per_game_per_day}
    omega_round_per_game_per_day = {each: np.mean(omega_round_per_game_per_day[each]) for each in
                                    omega_round_per_game_per_day}
    ...
    patamon_round_std = {each: np.std(patamon_round_per_game_per_day[each]) / len(patamon_round_per_game_per_day[each])
                         for each in patamon_round_per_game_per_day}
    patamon_round_per_game_per_day = {each: np.mean(patamon_round_per_game_per_day[each]) for each in
                                      patamon_round_per_game_per_day}
    plt.figure(figsize=(7, 5))
    ax = plt.subplot()
    ax.spines['right'].set_visible(False)
    ax.spines['top'].set_visible(False)
    ...
    plt.fill_between(
        np.arange(len(omega_day)),
        np.array([omega_round_per_game_per_day[each] for each in omega_day]) - np.array(
            [omega_round_std[each] for each in omega_day]),
        np.array([omega_round_per_game_per_day[each] for each in omega_day]) + np.array(
            [omega_round_std[each] for each in omega_day]),
        color=light_grey,
        alpha=0.2,
    ...
    plt.xticks(np.arange(len(temp_patamon_day))[temp_patamon_idx], np.array(temp_patamon_day)[temp_patamon_idx],
               fontsize=15)
    plt.ylim(0, 8)
    plt.yticks(np.arange(9), np.arange(9), fontsize=20)
    plt.tight_layout()
...
if __name__ == '__main__':
    file_base = "../../../Data/Behavior_Plot_Data/"
    pic_base = "../../Fig/"
    monkey = "all" # Omega, Patamon, all

    # # Fig 1: Basics
    # plotFig1B(file_base, pic_base, monkey)
    # plotFig1C(file_base, pic_base, monkey)
    #
    # # Fig 2: Strategy Fitting
    # plotFig2A(file_base, pic_base)
    # plotFig2B(file_base, pic_base, monkey)
    # plotFig2CDE(file_base, pic_base, monkey)
    # plotFig2F(file_base, pic_base, monkey)
    # # plotFig2FRadar(file_base, pic_base, monkey)
    #
    # # Fig 3: Strategy Patterns
    # plotFig3AB(file_base, pic_base, monkey)
    # plotFig3CD(file_base, pic_base, monkey)
    # plotFig3E(file_base, pic_base, monkey)
    # plotFig3F(file_base, pic_base, monkey, std_coef=3)
    #
    # # Fig 4: Plan (Planned Attack vs. Accidental Assumption & Suicide vs. Failure)
    # plotFig5A(file_base, pic_base, monkey)
    # plotFig56CD(file_base, pic_base, monkey)
    # plotFig5B6AB(file_base, pic_base, monkey)
    #
    # # Supp Fig. 2: Basic Statistics
    # plotBasicStatistics(file_base, pic_base)